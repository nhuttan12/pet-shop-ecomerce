import { ErrorMessage } from '@messages/error.messages';
import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';
import { UserErrorMessage } from '@user/messages/user.error-messages';
import { Type } from 'class-transformer';
import { IsOptional, IsInt, Min } from 'class-validator';

export class GetAllProductWithWishlistFlagWithUserIdDTO {
  @ApiProperty({ type: 'number' })
  @Type(() => Number)
  @IsInt({ message: UserErrorMessage.USER_ID_MUST_BE_INTEGER })
  @Min(1, { message: UserErrorMessage.USER_ID_MUST_BE_POSITIVE })
  userID: number;

  @ApiPropertyOptional({ default: 1 })
  @IsOptional()
  @Type(() => Number)
  @IsInt({ message: ErrorMessage.PAGE_MUST_BE_INTETER })
  @Min(1, { message: ErrorMessage.PAGE_SHOULD_NOT_A_NEGATIVE_NUMBER })
  page: number;

  @ApiPropertyOptional({ default: 10 })
  @IsOptional()
  @Type(() => Number)
  @IsInt({ message: ErrorMessage.LIMIT_MUST_BE_INTETER })
  @Min(10, { message: ErrorMessage.LIMIT_HAVE_AT_LEAST_10 })
  limit: number;
}
